...
def test_max_tokens_prompt_preserves_all_tool_results() -> None:
    async def run() -> None:
        first = TrackingTool(name="first")
        second = TrackingTool(name="second")
        with upstream_parsed_endpoint.context(
            outcomes=("tool_call_two", "final"),
            ...
            history = await agent.run("Use both tools.", session_id=0)

        tool_message = history[3]
        assert isinstance(tool_message, ToolMessage)
        assert set(tool_message.tool_responses) == {
            upstream_parsed_endpoint.TOOL_CALLS[0]["id"],
            upstream_parsed_endpoint.TOOL_CALLS[1]["id"],
        }
        assert tool_message.tool_responses[upstream_parsed_endpoint.TOOL_CALLS[0]["id"]] == "Context answer now."
