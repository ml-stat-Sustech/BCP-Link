...
def test_checkpoint_store_can_skip_completed_sample_without_output(tmp_path) -> None:
    store = CheckpointStore(
        tmp_path / "manifest.json",
        require_output_for_completed=False,
    )
    store._state["samples"] = {"000000": {"status": "completed"}}

    assert store.is_completed("000000", tmp_path / "missing.json")
