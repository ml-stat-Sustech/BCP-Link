...
class GenericDataLoader(BaseLoader):
    ...
    def __init__(self, 
        ...
        if config is not None:
            ...
            self.extra_keys = config.extra_keys
            return
    ...
    def _extract_record(self, record: dict[str, Any]) -> DataItem:
        ...
        if self.extra_keys is not None:
            missing = [key for key in self.extra_keys if key not in record]
            if missing:
                raise KeyError(f"missing extra_keys {missing!r} in record")
            extra = {key: record[key] for key in self.extra_keys}
        return record[self.input_key], extra, record.get(self.answer_key)
